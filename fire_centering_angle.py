...
Davranış:
  ...
  - Ateş var  → ateşin piksel offsetinden kaç derece dönmesi gerektiğini hesapla,
                o süre kadar dön, dur, tekrar kontrol et.

Hesaplama:
  FOV = 55°,  frame_width = ölçülen değer
  derece_basi_px = frame_width / FOV
  hata_derece    = hata_px / derece_basi_px
  donus_ms       = hata_derece * MS_PER_DEGREE

Sabitler:
  360° = 4800ms  →  1° = 13.33ms
  MOTOR_SPEED = 255  (Config.h'de)
  DEAD_ZONE_DEG = 3°  (±3° tolerans)
...
# ── Açı hesabı ────────────────────────────────────────────────────────────────
CAMERA_FOV_DEG  = 55.0   # kameranın yatay FOV'u (derece)
MS_PER_DEGREE   = 13.33  # 360°=4800ms → 1°=13.33ms
DEAD_ZONE_DEG   = 3.0    # ±3° tolerans — bu aralıkta "ortalandı" sayılır
CENTERED_HOLD   = 1.5    # saniye — stabil kalma süresi
MAX_TURN_DEG    = 45.0   # tek seferde maksimum dönüş (güvenlik sınırı)

# ── Tarama ────────────────────────────────────────────────────────────────────
SCAN_TURN_MS    = 600    # 45° tarama dönüş süresi (ms)
SCAN_WAIT_MS    = 500    # dönüş sonrası bekleme (ms)

# ── Sonraki dönüş öncesi bekleme ──────────────────────────────────────────────
POST_TURN_WAIT  = 0.5    # saniye — dönüş sonrası görüntünün stabilleşmesi için
...
def px_to_degrees(error_px: int, frame_width: int) -> float:
    """
    Piksel offsetini dereceye çevirir.
    error_px > 0 → ateş sağda → sağa dön
    error_px < 0 → ateş solda → sola dön
    """
    px_per_degree = frame_width / CAMERA_FOV_DEG
    return error_px / px_per_degree


def turn_by_degrees(ser, degrees: float):
    """
    Hesaplanan açı kadar döner ve durur.
    degrees > 0 → sağa, degrees < 0 → sola
    Maksimum MAX_TURN_DEG ile sınırlandırılır.
    """
    # Açıyı sınırla
    clamped = max(-MAX_TURN_DEG, min(MAX_TURN_DEG, degrees))
    if abs(clamped) != abs(degrees):
    ...
    send_cmd(ser, "STOP")
    time.sleep(POST_TURN_WAIT)   # görüntünün stabilleşmesi için bekle


# ── HUD ───────────────────────────────────────────────────────────────────────

def draw_hud(frame, fire_cx, fire_cy, x1, y1, x2, y2,
             conf, error_deg: float, is_centered: bool,
             centered_since: float, now: float, mode: str):
    h, w = frame.shape[:2]
    cx_frame = w // 2
    ...
    # Dead zone — dereceyi piksele çevir (görsel için)
    ...
    if is_centered:
        ...
        direction_str = "SAG" if error_deg > 0 else "SOL"
        label = f"{direction_str} DON: {abs(error_deg):.1f} derece"
        color = (0,80,255)
    ...
    try:
        while True:
            ...
            if is_centered:
                ...
                # Dead zone dışında → açı hesapla ve dön
                if centered_since != 0.0:
                    log("[DURUM] Dead-zone'dan çıkıldı, sayaç sıfırlandı.")
                centered_since = 0.0
                alert_fired    = False

                log(f"[HESAP] hata={error_deg:+.1f}°  →  dönüş hesaplanıyor")
                turn_by_degrees(ser, error_deg)
                # Dönüş bitti, bir sonraki frame'de tekrar ölç
                continue
